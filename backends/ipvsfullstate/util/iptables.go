...
func newInternal(exec exec.Interface, protocol Protocol, lockfilePath14x, lockfilePath16x string) IPTableInterface {
...
func (runner *iptablerunner) EnsureChain(table Table, chain Chain) (bool, error) {
...
func (runner *iptablerunner) FlushChain(table Table, chain Chain) error {
	fullArgs := makeFullArgs(table, chain)
...
func (runner *iptablerunner) DeleteChain(table Table, chain Chain) error {
	fullArgs := makeFullArgs(table, chain)
...
func (runner *iptablerunner) EnsureRule(position RulePosition, table Table, chain Chain, args ...string) (bool, error) {
...
func (runner *iptablerunner) DeleteRule(table Table, chain Chain, args ...string) error {
	fullArgs := makeFullArgs(table, chain, args...)
...
func (runner *iptablerunner) IsIPv6() bool {
	return runner.protocol == ProtocolIPv6
}

func (runner *iptablerunner) Protocol() Protocol {
	return runner.protocol
}

// SaveInto is part of Interface.
func (runner *iptablerunner) SaveInto(table Table, buffer *bytes.Buffer) error {
	runner.mu.Lock()
	defer runner.mu.Unlock()
...
func (runner *iptablerunner) Restore(table Table, data []byte, flush FlushFlag, counters RestoreCountersFlag) error {
	// setup args
	args := []string{"-T", string(table)}
...
func (runner *iptablerunner) RestoreAll(data []byte, flush FlushFlag, counters RestoreCountersFlag) error {
	// setup args
	args := make([]string, 0)
	return runner.restoreInternal(args, data, flush, counters)
}
...
func (runner *iptablerunner) restoreInternal(args []string, data []byte, flush FlushFlag, counters RestoreCountersFlag) error {
	runner.mu.Lock()
	defer runner.mu.Unlock()
...
func (runner *iptablerunner) run(op operation, args []string) ([]byte, error) {
	return runner.runContext(context.TODO(), op, args)
}

func (runner *iptablerunner) runContext(ctx context.Context, op operation, args []string) ([]byte, error) {
...
func (runner *iptablerunner) checkRule(table Table, chain Chain, args ...string) (bool, error) {
	if runner.hasCheck {
...
func (runner *iptablerunner) checkRuleWithoutCheck(table Table, chain Chain, args ...string) (bool, error) {
	iptablesSaveCmd := iptablesSaveCommand(runner.protocol)
...
func (runner *iptablerunner) checkRuleUsingCheck(args []string) (bool, error) {
...
func (runner *iptablerunner) Monitor(canary Chain, tables []Table, reloadFunc func(), interval time.Duration, stopCh <-chan struct{}) {
	for {
...
func (runner *iptablerunner) ChainExists(table Table, chain Chain) (bool, error) {
	fullArgs := makeFullArgs(table, chain)
...
func getIPTablesVersion(exec exec.Interface, protocol Protocol) (*utilversion.Version, error) {
...
func getIPTablesRestoreWaitFlag(version *utilversion.Version, exec exec.Interface, protocol Protocol) []string {
...
// in the form "X.X.X"
func getIPTablesRestoreVersionString(exec exec.Interface, protocol Protocol) (string, error) {
	// this doesn't access mutable state so we don't need to use the interface / runner
...
func (runner *iptablerunner) HasRandomFully() bool {
...
func (runner *iptablerunner) Present() bool {
